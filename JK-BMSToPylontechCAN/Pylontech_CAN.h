...
#define PYLON_CAN_BATTERY_SPECIFICATIONS_FRAME_ID   0x379 // Battery's Actual Capacity for Luxpower inverters (SNA5000)
...
struct PylontechCANSpecificationsFrameStruct {
    ...
    struct {
        ...
        uint8_t ReservedLowByte;            // "0x00"
        uint8_t ReservedHighByte;           // "0x00"
    } FrameData;
    void fillFrame(struct JKReplyStruct *aJKFAllReply) {
        (void) aJKFAllReply; // To avoid [-Wunused-parameter] warning
        ...
        FrameData.UnknownLowByte = 0x00;
        FrameData.UnknownHighByte = 0x00;
    }
};
